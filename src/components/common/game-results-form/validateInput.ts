...
import { hasValue, isAboveZero, isPositiveInteger } from "../../../utils/objects.ts";
...
export function validateFormInput(values: PastGame): string[] {
  ...
  if (!hasValue(values.bows) || !isPositiveInteger(values.bows)) missingFields.push("Bows");
  if (!hasValue(values.throwingWeapons) || !isPositiveInteger(values.throwingWeapons))
...
export function validateFormInputForStepper({
  ...
  if (!hasValue(duration) || !isAboveZero(duration)) missingFields.push("Duration");
  ...
  if (!hasValue(throwingWeapons) || !isPositiveInteger(throwingWeapons)) missingFields.push("Throwing Weapons");

  if (!hasValue(opponentArmies)) missingFields.push("Opponent Armies");

  if (!hasValue(victoryPoints)) missingFields.push("Victory Points");
  if (!hasValue(opponentVictoryPoints)) missingFields.push("Opponent's Victory Points");
...
export function findStepWithError({
  ...
  if (!hasValue(gameDate) || !hasValue(duration) || !isAboveZero(duration)) return 0;
