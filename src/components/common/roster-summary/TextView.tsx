...
export const RosterTextView = forwardRef<RosterTextViewHandlers, RosterTextViewProps>(
  ({ showArmyBonus, showUnitTotals }, ref) => {
    const { roster, getAdjustedMetaData } = useRosterInformation();
    const { additional_rules, special_rules, break_point } = armyListData[roster.armyList];
    ...
    const armyBonus = () => {
      ...
      const additionalRules = additional_rules?.map((rule) => `~ ${rule.description}\n`)?.join("  ");
      const specialRules = special_rules?.map((rule) => `*${rule.title}*\n${rule.description}\n`)?.join("  \n");

      return (
        header + (additionalRules ? `${additionalRules}  \n  ` : "") + (specialRules ? `${specialRules}  \n  ` : "")
      );
    };
    ...
    const createTextView = () => {
      const { might, will, fate, units, points, leader, bows, throwingWeapons } = getAdjustedMetaData();
