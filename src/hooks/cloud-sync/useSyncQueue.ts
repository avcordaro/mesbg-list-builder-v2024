...
export type EntityType = "rosters" | "groups" | "games" | "gameHistory" | "collections";
...
export function useSyncQueue() {
  ...
  const api = useApi();
  const apiMap: Record<EntityType, (data: SyncItemData, mappedIds?: Record<string, string>) => Promise<void | string>> =
    {
      ...
      collections: async (data: { origin: string; model: string; collection: ModelInventory }) => {
  ...
  const addItems = useCallback((items: Omit<SyncItem, "status">[]) => {
    setQueue((prev) => [...prev, ...items.map((i) => ({ ...i, status: "pending" as const }))]);
  }, []);
    ...
    for (const item of queue) {
      if (item.status === "pending") {
        ...
        try {
          ...
          setQueue((prev) =>
            prev.map((q) => (q.id === item.id ? { ...q, status: "failed", error: (err as Error).message } : q)),
          );
          setCompleted((c) => c + 1);
        }
      }
