...
export const useRosterInformation = (): RosterInformationFunctions => {
  ...
  const currentRoster = useRosterBuildingState((state): Roster => state.rosters.find(({ id }) => id === rosterId));
  const isCustomRoster = ["Custom: Good", "Custom: Evil"].includes(currentRoster?.armyList);

  function getModelIdsFromOptions(unit: Unit): string[] {
    const armyList = slugify(unit.army_list);
    if (unit.name === "Gandalf the White") {
      const hasPippinPassenger = !!unit.options.find(({ name, quantity }) => name === "Pippin" && quantity > 0);
      if (hasPippinPassenger) {
        return [`[${armyList}] peregrin-took`];
      }
    }
    if (unit.name === "Arwen") {
      const hasFrodoPassenger = !!unit.options.find(({ name, quantity }) => name === "Frodo Baggins" && quantity > 0);
      if (hasFrodoPassenger) {
        return [`[${armyList}] frodo-baggins`];
      }
    }
    if (unit.name === "Eowyn") {
      const hasMerryPassenger = !!unit.options.find(({ name, quantity }) => name === "Merry" && quantity > 0);
      if (hasMerryPassenger) {
        return [`[${armyList}] meriadoc-brandybuck`];
      }
    }
    if (unit.name === "Treebeard") {
      const hasMerryPassenger = !!unit.options.find(({ name, quantity }) => name === "Merry & Pippin" && quantity > 0);
      if (hasMerryPassenger) {
        return [`[${armyList}] peregrin-took`, `[${armyList}] meriadoc-brandybuck`];
      }
    }
    return [];
  }
  ...
  function getSetOfModelIds(roster: Roster = currentRoster) {
    ...
    return roster.warbands
      ...
      .filter((item, index, self) => self.findIndex((other) => other === item) === index);
  }

  function getSetOfModelIdsInWarband(warbandId: string, roster: Roster = currentRoster) {
    if (!roster) return [];
    return roster.warbands
  ...
  function getAdjustedMetaData(roster: Roster = currentRoster): Roster["metadata"] {
    if (roster.armyList === "Rivendell" && getSetOfModelIds(roster).includes("[rivendell] elrond")) {
    ...
    if (roster.armyList === "The Three Trolls") {
      return {
        ...roster.metadata,
        points: roster.metadata.points + (roster.metadata.tttSpecialUpgrades?.length * 50 || 0),
      };
    }

    if (roster.armyList === "Erebor & Dale") {
      const units = roster.warbands.flatMap((w) => w.units).filter(isSelectedUnit);
      return {
        ...roster.metadata,
        bowLimit: units.filter((u) => u.profile_origin === "Kingdoms of Men").reduce((s, u) => s + u.quantity, 0),
        throwLimit: units.filter((u) => u.profile_origin === "Dwarven Holds").reduce((s, u) => s + u.quantity, 0),
      };
    }
  ...
  function canSupportMoreWarbands(roster: Roster = currentRoster): boolean {
    ...
    if (roster.armyList === "Custom: Good" || roster.armyList === "Custom: Evil") return true;
    ...
    const stillAvailableWarbandLeaders = Object.values(mesbgData)
      .filter(({ unit_type }) => unit_type.includes("Hero") || unit_type === "Siege Engine")
      .filter((unit) => unit.army_list === roster.armyList && handleSpecialRestriction(unit))
      .filter((unit: Unit) => !unit.unique || !setOfModelIds.includes(unit.model_id));
    ...
    const warbandsInNeedOfHero = roster.warbands.filter((warband) => !warband.hero).length;
