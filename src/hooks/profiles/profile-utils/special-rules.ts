...
export function getAdditionalSpecialRules(unit: SelectedUnit) {
  ...
  if (unit.unit_type === "Siege Engine") {
    const siegeEngineUpgrades = ["Flaming Ammunition", "Swift Reload", "Superior Construction", "Severed Heads"];
    return unit.options
      .filter(({ quantity }) => quantity)
  ...
  if (["Gandalf the White", "Eowyn", "Treebeard", "Arwen"].includes(unit.name)) {
    if (unit.options.find((option) => option.type === "passenger" && option.quantity > 0)) {
